...
const COMPARISON_ROWS = [
  { label: "24/7 AI legal guidance", basic: true, premium: true, ultimate: true },
  { label: "Legal document templates", basic: true, premium: true, ultimate: true },
  { label: "Lawyer consultations", basic: false, premium: "2 / month", ultimate: "Unlimited" },
  { label: "Contract review by an attorney", basic: false, premium: true, ultimate: true },
  { label: "Legal expense cover", basic: false, premium: false, ultimate: "Up to R500,000" },
  { label: "Court representation support", basic: false, premium: false, ultimate: true },
];
...
function ComparisonCell({ value }) {
  if (value === true) return <CheckCircleIcon color="success" fontSize="small" />;
  if (value === false) return <RemoveCircleOutlineIcon color="disabled" fontSize="small" />;
...
export function Plans() {
  ...
  useEffect(() => {
    policyService
      .listPlans()
      .then(setPlans)
      .catch(() => setError("We couldn't load our plans right now. Please try again later."))
    ...
    <Container maxWidth="lg" sx={{ py: 8 }}>
      <Box sx={{ textAlign: "center", mb: 6 }}>
        ...
        <Typography variant="body1" color="text.secondary" sx={{ maxWidth: 640, mx: "auto" }}>
          Straightforward monthly pricing with no hidden fees. Upgrade,
        ...
        <Table>
          <TableHead>
            <TableRow>
              ...
              <TableCell align="center" sx={{ fontWeight: 700 }}>Premium</TableCell>
              <TableCell align="center" sx={{ fontWeight: 700 }}>Ultimate</TableCell>
            </TableRow>
          </TableHead>
          <TableBody>
            {COMPARISON_ROWS.map((row) => (
              <TableRow key={row.label}>
                ...
                <TableCell align="center"><ComparisonCell value={row.basic} /></TableCell>
                <TableCell align="center"><ComparisonCell value={row.premium} /></TableCell>
                <TableCell align="center"><ComparisonCell value={row.ultimate} /></TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
